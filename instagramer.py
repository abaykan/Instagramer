...
import os, sys, json,requests, urllib.request,time
from optparse import OptionParser

def getUserDetails(username):
	try:
		data = requests.get("https://www.instagram.com/"+username+"/?__a=1")
	...
	f = open(username + "/info.json", "w")
	f.write(json.dumps(info, indent=4) + "\n")
	f.close()

	if options.getmedia == None:
		...
		print("Followers / Followings \t: " + str(output["graphql"]["user"]["edge_followed_by"]["count"]) + " Followers / " + str(output["graphql"]["user"]["edge_follow"]["count"]) + " Followings")
		...
		getUserMedia(output["graphql"]["user"]["id"], str(output["graphql"]["user"]["username"]), str(output["graphql"]["user"]["edge_owner_to_timeline_media"]["count"]), output["graphql"]["user"]["is_private"], next_page)

def getUserMedia(user_id, username, media_count, is_private, next_page=0, end_cursor="", itung=0):
	if is_private == True:
		print("Error: Private User detected! Can't get " + username + "'s media!")
		sys.exit()
	...
	if next_page == 1:
		try:
			data = requests.get("https://www.instagram.com/graphql/query/?query_id=17888483320059182&id=" + user_id + "&first=50&after=" + str(end_cursor))
		except requests.ConnectionError:
			print("\nError: Connection Error")
			sys.exit()
		...
		try:
			data = requests.get("https://www.instagram.com/graphql/query/?query_id=17888483320059182&id=" + user_id + "&first=50&after=")
		except requests.ConnectionError:
			print("\nError: Connection Error")
			sys.exit()
	...
	for x in output["data"]["user"]["edge_owner_to_timeline_media"]["edges"]:
		...
		if media_type == "Video":
			data_video = urllib.request.urlopen("https://www.instagram.com/p/" + str(x["node"]["shortcode"]) + "/?__a=1").read()
			output_video = json.loads(data_video)
			filename = output_video["graphql"]["shortcode_media"]["video_url"].split('?')[0]
			filename = filename.split('/')[-1]
			urllib.request.urlretrieve(output_video["graphql"]["shortcode_media"]["video_url"], username + "/" + filename)
		else:
		...
		print("Shortcode \t: " + str(x["node"]["shortcode"]) + " | " + str(f"\u001b]8;;{target}\u001b\\{text}\u001b]8;;\u001b\\"))
		if media_type == "Video":
